...
def actionables(text,detected_language):
    spacy_models = {
...
def analyze_text_combined(text):
    ...
    with ThreadPoolExecutor(max_workers=3) as executor:
        # Schedule the functions to run in parallel and return Futures
        future_sentiment = executor.submit(sentiment, cleaned_text, detected_language)
        future_verbs = executor.submit(actionables, cleaned_text, detected_language)
        future_profanities = executor.submit(profanity, cleaned_text)

        # Wait for the results
        sentiments = future_sentiment.result()
        verbs = future_verbs.result()
        profanities = future_profanities.result()

    # Check profanities to adjust sentiment
    if profanities != []:
        sentiments = "Negative"

    # Adjust detected language for readability
    if detected_language == "urd_Arab":
        detected_language = "Urdu"
    elif detected_language == "eng_Latn":
